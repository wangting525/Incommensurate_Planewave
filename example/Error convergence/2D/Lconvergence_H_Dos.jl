...
# Generate EcL 
vecEcL = [10,20,30,40,50]
lengthL = length(vecEcL)

# test function
g_fermi_beta1mu10(x) = x*(1+exp((x-10)))^(-1)
g(x) = g_fermi_beta1mu10(x)

dos_fermi_beta1mu10 = zeros(Float64,lengthL);
for i = 1 : lengthL
	EcL = vecEcL[i]
	EcW = 10.0
	neigs=10
    ...
    dos_fermi_beta1mu10[i] = Coe*(1/EcL^2)*sum(d_fermi_beta1mu10_EcW)
end
...
dos_fermi_beta1mu10_exact[i] = Coe*(1/EcL^2)*sum(d_fermi_beta1mu10_EcW_exact)


# plot the error
Lerror_fermi_beta1mu10 = zeros(Float64,lengthL)
for i = 1 : lengthL
    Lerror_fermi_beta1mu10[i] = abs(dos_fermi_beta1mu10_exact - dos_fermi_beta1mu10[i])
end   
plot(vecEcL, Lerror_fermi_beta1mu10, xaxis=:log, yaxis=:log, label="Error", xlabel="L", ylabel= "Error", ls=:solid, legend=:bottomleft, shape=:circle, markersize=6, lw=3, tickfontsize=15, legendfontsize=15, guidefontsize=15)
