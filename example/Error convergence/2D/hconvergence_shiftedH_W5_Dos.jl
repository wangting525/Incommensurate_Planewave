...
plot(vechinv, herror, xaxis = :log, yaxis=:log, label=[L"\beta=1" L"\beta=0.1"], xlabel=L"h^{-1}", ylabel= "Error", legend= :bottomleft, shape=[:circle :utriangle], markersize=6, lw=3, tickfontsize=15, legendfontsize=13, guidefontsize=15)
plot!(vechinv, 20*vech.^2, xaxis=:log, yaxis=:log, label= L"\mathcal{O}(h^{-2})", ls=:dash, lw=3, c=:black)